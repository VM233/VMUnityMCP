...
| UI Toolkit static audit | `unity_uitoolkit_audit_uxml_layout` | `uitoolkit/audit-uxml-layout` | Find unconsumed authored element names, fully fixed flex partitions that should leave one flexible remainder, fixed cross-axis content wrappers inside single-axis ScrollViews, layout-only manually centered containers, removable single-child centering wrappers, visually inert centered-label stretching or growth, repeated inline layout variants, and inline declarations already supplied by a loaded USS default, including implicit UI Toolkit classes. |
