...
| UI Toolkit static audit | `unity_uitoolkit_audit_uxml_layout` | `uitoolkit/audit-uxml-layout` | Find layout-only manually centered containers, visually inert centered-label stretching, repeated inline layout variants, and inline declarations already supplied by a loaded USS default, including implicit UI Toolkit classes. |
...
- The UI Toolkit static audits are explicit read-only tools in every project. Automatic import and filesystem-write warnings are opt-in through `ProjectSettings/UnityMCPUIToolkitAudit.json`, so installing the package does not impose an authoring policy on unrelated projects. The configuration accepts `automaticAudit.ussSingleUseStyles`, `automaticAudit.uxmlLayoutContracts`, `assetRoots`, `runtimeSourceRoots`, and `excludePaths`. Reasoned local exceptions use `/* uss-audit: allow-single-use <reason> */`, `<!-- uxml-layout-audit: allow-manual-center <reason> -->`, `<!-- uxml-layout-audit: allow-repeated-inline <reason> -->`, `<!-- uxml-layout-audit: allow-redundant-inline <reason> -->`, and `<!-- uxml-layout-audit: allow-inert-text-stretch <reason> -->`.
